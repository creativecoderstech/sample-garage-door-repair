...
// In the marketing demo the admin is embedded as an iframe with ?preview=true.
// Use a 3-second interval there so new chats appear quickly during a live demo.
// Production keeps the 20-second default to avoid unnecessary load.
const isPreviewDemo =
  typeof window !== 'undefined' &&
  new URLSearchParams(window.location.search).get('preview') === 'true';
const POLL_INTERVAL_MS = isPreviewDemo ? 3_000 : 20_000;
...
export function ChatInquiriesAdmin() {
  ...
  // refetchIntervalInBackground: true keeps the poll running even when the
  // iframe loses window focus (e.g. when the visitor clicks in the chat iframe).
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const queryOpts = {
    query: { refetchInterval: POLL_INTERVAL_MS, refetchIntervalInBackground: true },
  } as any;
