...
function adminOrigin(env: AuthEnv, requestUrl: string): string {
  ...
  if (url.hostname === "admin.sample-handyman.samples.creativecoders.tech") {
    return "https://admin.sample-handyman.samples.creativecoders.tech";
  }
...
export async function readSessionUser(
  ...
  if (!secret) return null; // no secret → unauthenticated
  const token = getCookie(c, SESSION_COOKIE);
  if (!token) return null; // no session cookie → unauthenticated
