...
const SETTINGS = {
  ...
  notifyFromEmail: 'bookings@pennylanehomesolutions.com',
  notifyFromName: 'Penny Lane Home Solutions',
  thumbtackRating: '4.9',
...
describe('SettingsAdmin locked fields', () => {
  it('renders values as read-only text with no inputs until Edit is clicked', () => {
    ...
    expect(screen.getByText('(706) 244-8550')).toBeTruthy();
  ...
  it('unlocks only the edited field, saves the change, and relocks', () => {
    ...
    fireEvent.change(inputs[0], { target: { value: '(770) 555-0000' } });
    fireEvent.click(screen.getByRole('button', { name: /Save/ }));
    expect(mutateMock).toHaveBeenCalledTimes(1);
    const sent = mutateMock.mock.calls[0][0].data;
    expect(sent.phone).toBe('(770) 555-0000');
    expect(sent.ownerEmail).toBe(SETTINGS.ownerEmail); // untouched fields preserved
    // relocked after save
    expect(document.querySelectorAll('input').length).toBe(0);
    expect(screen.getByText('(770) 555-0000')).toBeTruthy();
  ...
  it('cancel and Escape restore the original value without saving', () => {
    ...
    expect(screen.getByText('Penny Lane Home Solutions')).toBeTruthy();
