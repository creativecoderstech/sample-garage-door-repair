...
import { CheckCircle2, ExternalLink, ImagePlus, Loader2, Sparkles, Star, X } from 'lucide-react';
import {
  TIME_WINDOW_LABELS,
...
export function BookingForm() {
    ...
    createRequest.mutate(
      {
        data: {
          ...
          preferredTime: data.preferredTime as
            | 'morning'
            | 'afternoon'
            | 'evening'
            | undefined,
      ...
      },
      {
        onSuccess: () => {
          ...
          toast({
            ...
            description: 'Brandon will review your request and confirm the time.',
          });
        },
        onError: () => {
          toast({
  ...
  if (submitted) {
    return (
      <div className="relative text-center py-14 px-8 bg-gradient-to-br from-accent/10 to-accent/5 rounded-3xl border-2 border-accent/20 shadow-xl overflow-hidden" data-testid="message-booking-success">
        {/* Close button */}
        <button
        ...
        <p className="text-muted-foreground text-lg leading-relaxed max-w-md mx-auto">
          Brandon will review your request and confirm the time. You'll get a notification once it's confirmed.
        </p>
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-7" data-testid="form-booking">
          ...
          render={({ field }) => (
            <FormItem>
              ...
              <p className="text-xs text-muted-foreground">
                Needed so we can email you when Brandon confirms your booking.
              </p>
              <FormMessage />
            </FormItem>
          )}
            ...
            <FormItem>
              ...
              <VoiceInput
                ...
                onTranscript={(text) => {
                  ...
                  form.setValue('description', current ? `${current.trim()} ${text}` : text, {
                    shouldValidate: true,
                    shouldDirty: true,
                  });
                }}
          ...
          render={({ field }) => (
            <FormItem>
              ...
              {urgency === 'urgent' ? (
                ...
                <p className="text-xs text-muted-foreground">
                  Helps Brandon know how to prioritize — not a guaranteed same-day visit.
                </p>
              )}
              <FormMessage />
            </FormItem>
        ...
        <div className="space-y-3">
          <div>
            <p className="text-sm font-bold text-foreground">Photos (optional)</p>
            <p className="text-xs text-muted-foreground mt-1">
              Add up to {MAX_PHOTOS} photos of the job (JPEG, PNG, or WebP · 5MB each).
            </p>
          </div>
          <label className="flex flex-col items-center justify-center gap-2 rounded-xl border-2 border-dashed border-border bg-muted/20 px-4 py-8 cursor-pointer hover:border-primary/40 transition-colors">
          ...
          {photoError && (
            <p className="text-sm text-destructive font-medium">{photoError}</p>
          )}
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel className="text-sm font-bold text-foreground">Preferred Date (optional)</FormLabel>
                <FormControl>
                  <Input 
                    type="date"
                    min={minDate}
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel className="text-sm font-bold text-foreground">Preferred Time Window (optional)</FormLabel>
                <Select
