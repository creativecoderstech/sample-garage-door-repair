...
type Env = {
  ...
  ENVIRONMENT: string;
  /** OpenAI API key for the chat assistant (gpt-5.4-mini) */
  OPENAI_API_KEY: string;
  /** OpenAI-compatible base URL, e.g. https://api.openai.com/v1 */
  OPENAI_BASE_URL: string;
  /** Production admin dashboard origin, e.g. https://admin.pennylanehomesolutions.com */
  ADMIN_ORIGIN?: string;
  /** Public marketing site origin, e.g. https://pennylanehomesolutions.com */
...
const DEFAULT_ADMIN_ORIGIN = "https://admin.pennylanehomesolutions.com";
...
app.get("/api/auth/dev-login", async (c) => {
  ...
  if (!user) {
    return c.json(
      { error: "No active super_admin found. Run: wrangler d1 migrations apply penny-lane-db-dev --local --env dev" },
      500,
    );
  }
...
app.get("/api/healthz", (c) => {
  c.header("X-Penny-Lane-Env", c.env.ENVIRONMENT || "production");
...
app.get("/api/service-requests", async (c) => {
  ...
  const rows = results ?? [];
  const photoMap = await loadPhotoUrlsByRequestId(
    c.env.DB,
    rows.map((r) => r.id),
  );
  return c.json(
    ListServiceRequestsResponse.parse({
      items: rows.map((r) =>
        serializeServiceRequest(r, photoMap.get(r.id) ?? []),
      ),
...
app.post("/api/service-requests", async (c) => {
  ...
  try {
    ...
    console.error("Service request photo upload failed", err);
    await Promise.all(
...
app.post("/api/chat", async (c) => {
  ...
  const ctx: AssistantContext = {
    ...
    phone: phoneRow?.value?.trim() || "(706) 244-8550",
  };

  try {
    const reply = await getChatReply(c.env.OPENAI_API_KEY, c.env.OPENAI_BASE_URL, history, ctx);
    if (inquiryId != null) {
    ...
    return c.json(SendChatMessageResponse.parse({ reply }));
  } catch (err) {
...
const DEFAULT_PHONE = "(770) 555-1234";
...
async function serializeSiteSettings(db: D1Database) {
  const [settings, heroKey, ttRating, ttCount, trRating, trCount, googleReviewUrl, googlePlaceId] =
    await Promise.all([
...
app.put("/api/settings", async (c) => {
  ...
  if (parsed.data.notifyFromName !== undefined) {
    stmts.push(
      upsertStmt(
        "notify_from_name",
        parsed.data.notifyFromName.trim() || "Penny Lane Home Solutions",
      ),
    );
  }
...
export default {
  async fetch(request: Request, env: Env, ctx: ExecutionContext) {
    ...
    if (url.hostname === "www.pennylanehomesolutions.com") {
      url.hostname = "pennylanehomesolutions.com";
