...
export async function readNotifySettings(db: D1Database) {
  ...
  return {
    ownerEmail,
    notifyFromEmail,
    notifyFromName: notifyFromName || "Penny Lane Home Solutions",
    phone: phone || "(770) 555-1234",
  };
}
...
export async function notifyUserInvite(
  ...
  const text = [
    `You've been invited to Penny Lane Home Solutions as ${roleLabel}.`,
    "",
  ...
  const html = `
    <p>You've been invited to <strong>Penny Lane Home Solutions</strong> as <strong>${escapeHtml(roleLabel)}</strong>.</p>
    <p><a href="${escapeHtml(loginUrl)}">Sign in with Google</a></p>
    <p>Use the same Google account email this invite was sent to.</p>
  `;
  const result = await sendEmail(env, {
    to: args.to,
    subject: `You're invited to Penny Lane Admin (${roleLabel})`,
    text,
    html,
  });
...
export async function notifyClientJobCompleted(
  ...
  const settings = await readNotifySettings(env.DB);
  const reviewUrl = "https://pennylanehomesolutions.com/#testimonials";

  const text = [
    `Hi ${booking.name},`,
    "",
    `Your ${booking.service} job is complete — thank you so much for choosing Penny Lane Home Solutions! It was a pleasure taking care of your home.`,
    "",
    ...
    "",
    "Warm regards,",
    "Brandon — Penny Lane Home Solutions",
  ].join("\n");
  ...
  const html = `
    <div style="font-family:Outfit,Inter,sans-serif;color:#0f291c">
      ...
      <p>Your <strong>${escapeHtml(booking.service)}</strong> job is complete — thank you so much for choosing <strong>Penny Lane Home Solutions</strong>! It was a pleasure taking care of your home.</p>
      ...
      <p>Warm regards,<br>Brandon — Penny Lane Home Solutions</p>
    </div>
  `;
...
export async function notifyClientBookingConfirmed(
  ...
  const text =
    `Hi ${booking.name}, your ${booking.service} request with Penny Lane Home Solutions is confirmed for ${when}. ` +
