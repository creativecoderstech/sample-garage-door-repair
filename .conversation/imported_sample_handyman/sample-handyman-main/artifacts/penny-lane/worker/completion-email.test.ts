  ...
  const row = await db()
    .prepare(
      `INSERT INTO bookings (name, email, phone, service, description, scheduled_date, scheduled_time, status, source)
       VALUES ('Casey Customer', ?, '(770) 555-0000', 'Drywall Repair', 'Patch hallway wall', '2026-08-20', 'morning', ?, 'web')
       RETURNING id`,
    )
...
describe("completion thank-you email", () => {
    ...
    await db()
      .prepare(
        `INSERT INTO site_settings (key, value) VALUES
           ('notify_from_email', 'no-reply@pennylanehomesolutions.com'),
  ...
  it("sends the review-request email on transition to completed", async () => {
    ...
    expect(msg.text).toContain("https://pennylanehomesolutions.com/#testimonials");
    expect(msg.html).toContain("https://pennylanehomesolutions.com/#testimonials");
