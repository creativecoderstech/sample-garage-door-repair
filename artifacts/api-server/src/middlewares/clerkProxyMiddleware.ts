...
export function getClerkProxyHost(req: {
  ...
  const trustedHosts = new Set(
    [process.env.PUBLIC_SITE_ORIGIN, process.env.REPLIT_DEV_DOMAIN]
      .filter(Boolean)
      .flatMap((value) => {
        try {
  ...
  const direct = req.headers.host?.trim();
  if (firstHop && trustedHosts.has(firstHop)) return firstHop;
  if (direct && trustedHosts.has(direct)) return direct;
  return undefined;
}
