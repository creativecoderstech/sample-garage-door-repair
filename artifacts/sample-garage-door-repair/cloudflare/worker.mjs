...
async function staff(request, env) {
  // The deployed demo is read-only for staff APIs. This explicit test/local
  // switch cannot enable unauthenticated access on an internet-facing host.
...
function suggest(message) { if (/spring|torsion|extension/i.test(message)) return "springs"; if (/opener|remote|keypad|sensor|motor/i.test(message)) return "opener"; if (/new (?:garage )?door|replace|install/i.test(message)) return "installation"; if (/maint|inspect|tune|lubricat|annual/i.test(message)) return "maintenance"; if (/off.?track|track|roller|cable|hinge|noisy|slow/i.test(message)) return "repair"; return "Service assessment"; }
...
async function getCustomerCareContext(env) {
  ...
  try { if (env.DB) { row = await env.DB.prepare("SELECT settings_json, verified FROM business_settings WHERE id=1").first(); publicContent = await getPublicContent(env); } } catch { row = null; publicContent = []; }
    ...
    if (path.startsWith("/api/garage/admin/content/")) {
      ...
      if (request.method === "PUT") {
        try {
          ...
          if (current.kind !== "page" && value.slug && value.slug !== current.slug && Array.isArray(value.aliases)) value.aliases = [...new Set([...value.aliases, current.slug])].slice(0, 25);
          if (value.status === "published" && value.verificationStatus === "verified" && value.verificationAcknowledged !== true) return json({ error: "Explicit verification acknowledgement is required before publishing business claims." }, 400);
  ...
  if (path === "/api/garage/requests" || path.startsWith("/api/garage/requests/") || path === "/api/garage/dashboard" || path === "/api/garage/settings") {
    ...
    if (path === "/api/garage/settings" && request.method === "PATCH") {
      try {
        ...
        const verified = explicitlyVerified && update.verificationAcknowledged === true
          ? true
          : explicitlyUnverified || sensitiveChanged
            ? false
            : current?.verified === 1;
  ...
  async fetch(request, env, context) {
    ...
    try {
      ...
      if ((request.method === "GET" || request.method === "HEAD") && publicPath === "/sitemap.xml") {
        const content = await getPublicContent(env);
        return new Response(request.method === "HEAD" ? null : renderSitemap(request.url, content), { headers: { "content-type": "application/xml; charset=utf-8", "cache-control": "no-store", "x-robots-tag": "noindex, nofollow, noarchive" } });
      }
