...
const uploadRules = new Map([["image/jpeg", 5 * 1024 * 1024], ["image/png", 5 * 1024 * 1024], ["image/webp", 5 * 1024 * 1024], ["video/mp4", 100 * 1024 * 1024], ["video/quicktime", 100 * 1024 * 1024], ["video/webm", 100 * 1024 * 1024]]);
...
async function validUploadCapability(env, requestId, token, allowCompleted = false) {
  ...
  return row?.upload_capability === token && (allowCompleted || !row.completed_at) && Date.now() - Date.parse(row.created_at) < 60 * 60 * 1000;
}
export function validateWebhook(raw, env = {}) {
  let url; try { url = new URL(raw); } catch { throw new Error("Enter a valid HTTPS webhook URL."); }
...
function webhookAuthorized(raw, env) { try { validateWebhook(raw, env); return true; } catch { return false; } }
...
async function deliverNotification(env, requestId) {
  ...
  if (!settings?.enabled || !settings.webhook_url) { await env.DB.prepare("UPDATE notification_outbox SET status='unconfigured',last_error='Notification destination is not configured.',updated_at=? WHERE request_id=?").bind(new Date().toISOString(), requestId).run(); return "unconfigured"; }
  try {
    ...
    const response = await fetch(destination, { method: "POST", redirect: "error", headers: { "content-type": "application/json" }, body: JSON.stringify({ event: "service_request.created", eventId: `service-request:${requestId}`, request: { ...mapRequest(request), attachments: details.attachments } }), signal: AbortSignal.timeout(15000) });
    if (!response.ok) throw new Error(`Destination returned HTTP ${response.status}.`);
    await env.DB.prepare("UPDATE notification_outbox SET status='delivered',attempts=attempts+1,last_error=NULL,delivered_at=?,next_attempt_at=NULL,updated_at=? WHERE request_id=?").bind(new Date().toISOString(), new Date().toISOString(), requestId).run();
    return "delivered";
  } catch (error) {
    await env.DB.prepare("UPDATE notification_outbox SET status='failed',attempts=attempts+1,last_error=?,next_attempt_at=NULL,updated_at=? WHERE request_id=?").bind(`${String(error.message || "Delivery failed.").slice(0, 450)} Staff must retry delivery manually.`, new Date().toISOString(), requestId).run();
    return "failed";
  }
}
export function publicSettings(row, dependencies = {}) {
...
export async function getPublicContent(env) {
  if (!env.DB) return [];
...
function validateContent(value) {
  ...
  const lengths = { title: 160, summary: 500, body: 20000, imageUrl: 2048, imageAlt: 300, beforeImageUrl: 2048, seoTitle: 160, seoDescription: 320, serviceCode: 100 };
  ...
  if (!Number.isInteger(value.sortOrder) || value.sortOrder < -100000 || value.sortOrder > 100000 || !["draft", "published"].includes(value.status) || !["unverified", "verified"].includes(value.verificationStatus) || typeof value.featured !== "boolean") return "Invalid publication fields.";
  if (value.parentId !== undefined && value.parentId !== null && (typeof value.parentId !== "string" || value.parentId.length > 100)) return "Invalid parent.";
  ...
  if (!Array.isArray(value.serviceFaqs) || value.serviceFaqs.length > 20 || value.serviceFaqs.some(item => !item || typeof item.question !== "string" || !item.question || item.question.length > 300 || typeof item.answer !== "string" || !item.answer || item.answer.length > 1000)) return "Invalid service FAQs.";
...
async function validateContentRelations(env, value, currentId = null) {
  ...
  let cursor = parent; const visited = new Set();
  while (cursor) { if (cursor.id === currentId) return "Parent selection would create a cycle."; if (visited.has(cursor.id)) break; visited.add(cursor.id); cursor = cursor.parent_id ? rows.find(row => row.id === cursor.parent_id) : null; }
  const routes = new Set([value.slug, ...value.aliases]);
...
export function selectApprovedService(message, availableCodes) {
...
function shouldRecommendServiceRequest(message, safetyLevel, reply = "") { const normalized = message.trim(); if (!normalized || /^(?:hi|hello|hey|good morning|good afternoon|good evening|thanks|thank you|thx|ty|how are you|how'?s it going)[!?. ]*$/i.test(normalized)) return false; return safetyLevel === "urgent" || actionableGarageTerms.test(normalized) || businessFollowupTerms.test(normalized) || uncertaintyTerms.test(reply) || !garageIssueTerms.test(normalized); }
function withServiceRequestGuidance(reply, recommended) { return recommended && !/service request/i.test(reply) ? `${reply}\n\nIf you’d like the business to review the details, you can start a service request here.` : reply; }
function casualCustomerCareReply(message, businessName) { const normalized = message.trim().toLowerCase(); if (normalized.length > 100 || garageIssueTerms.test(normalized)) return null; if (/^(thanks|thank you|thx|ty|appreciate it)[!. ]*$/i.test(normalized)) return "You’re welcome. If anything changes with the door, tell me what you notice and I’ll help with the next step."; if (/^(how are you|how'?s it going|hru|you good)[?!., ]*$/i.test(normalized)) return `Thanks for asking. I’m here with customer care at ${businessName}. What’s the garage door doing today—stuck, noisy, slow, or refusing to open?`; if (/^(hi|hello|hey|good morning|good afternoon|good evening)[!. ]*$/i.test(normalized)) return `Hi! You’ve reached Maya with customer care for ${businessName}. You don’t need to know the repair name—just tell me what the door is doing.`; return null; }
function cleanHistory(value) { return Array.isArray(value) ? value.filter(item => item && (item.role === "user" || item.role === "assistant") && typeof item.content === "string" && item.content.trim()).slice(-12).map(item => ({ role: item.role, content: item.content.trim().slice(0, 1000) })) : []; }
async function getCustomerCareContext(env) {
  let row = null, notification = null, publicContent = [];
  try { if (env.DB) { [row, notification, publicContent] = await Promise.all([env.DB.prepare("SELECT settings_json, verified FROM business_settings WHERE id=1").first(), env.DB.prepare("SELECT enabled,webhook_url,destination_verified,tested_at FROM notification_settings WHERE id=1").first(), getPublicContent(env)]); } } catch { row = null; notification = null; publicContent = []; }
...
async function handleApi(request, url, env) {
  ...
  if (path === "/api/garage/admin/session") {
    try { return json(await staffActor(request, env)); } catch (e) { return json({ error: e.message }, e.status || 401); }
  }
  if (path === "/api/garage/admin/access" || path.startsWith("/api/garage/admin/access/")) {
    try {
      ...
      if (path.startsWith("/api/garage/admin/access/") && request.method === "DELETE") { await revokeAccess(env, actor, decodeURIComponent(path.slice("/api/garage/admin/access/".length))); return new Response(null, { status: 204 }); }
  ...
  if (path === "/api/garage/requests" && request.method === "POST") {
    ...
    try {
      ...
      const createdAt = new Date().toISOString(), uploadCapability = `${crypto.randomUUID()}${crypto.randomUUID()}`;
    ...
    } catch (e) { return json({ error: e.message || "Unable to send request." }, 400); }
  }
  ...
  if (uploadMatch) {
    ...
    if (uploadMatch[2] && request.method === "POST") {
      ...
      await env.DB.prepare("DELETE FROM request_attachments WHERE request_id=? AND status='pending' AND created_at < ?").bind(requestId, new Date(Date.now() - 900000).toISOString()).run();
  ...
  if (path === "/api/garage/assistant" && request.method === "POST") {
    ...
    try { const body = await bodyJson(request); if (!await verifyTurnstile(body.turnstileToken, "assistant", request, env)) return json({ error: "Verification failed. Please try again." }, 403); const message = String(body.message || "").trim().slice(0, 1000); if (!message) return json({ error: "Ask a question about your garage door." }, 400); const history = cleanHistory(body.history), context = await getCustomerCareContext(env), candidate = selectApprovedService([...history.map(item => item.content), message].join("\n"), context.catalog.map(service => service.serviceCode || service.slug)), catalog = context.catalog.find(service => (service.serviceCode || service.slug) === candidate), safe = fallbackAssistant(message, candidate); if (!env.AI) return json({ error: "Maya is unavailable because the AI provider is not configured.", safetyGuidance: safe.reply, ...safe }, 503); const casualReply = casualCustomerCareReply(message, context.settings.businessName); if (casualReply) return json({ reply: casualReply, safetyLevel: "safe", suggestedService: "Service assessment", serviceRequestRecommended: false }); try { const out = await env.AI.run("@cf/meta/llama-3.1-8b-instruct", { messages: [{ role: "system", content: [`You are Maya, the friendly customer-care coordinator for ${context.settings.businessName}. Speak warmly, naturally, and plainly as the first helpful voice a homeowner reaches.`, "You are an AI-assisted service-information tool, not a technician or emergency service. Never imply a human is monitoring the conversation.", "Answer business, service, estimate, coverage, timing, review, FAQ, and booking questions only from the authoritative context below. If the context does not confirm an answer, say that plainly and invite the customer to start a service request. Never guess.", "Keep replies to 2–5 short sentences and ask at most one useful follow-up question. Connect a symptom to one verified service when possible rather than listing everything.", "A service request is not an appointment. Never promise coverage, timing, arrival, final price, warranties, credentials, refunds, or availability.", "SAFETY: Never instruct customers to adjust, unwind, cut, replace, or pull springs, cables, bottom brackets, tracks, or other high-tension parts. For crooked, hanging, fallen, off-track, unusually heavy, or spring-damaged doors, tell them to stop using the door and keep people, pets, and vehicles clear.", context.text].join("\n\n") }, ...history, { role: "user", content: message }] }); let reply = String(out?.response || "").trim(); if (!reply || /(?:appointment (?:is |has been )?confirmed|guaranteed|we can be there|available today|licensed and insured|open 24)/i.test(reply) || (context.settings.verificationStatus !== "verified" && /(?:we serve|our hours|our warranty|we are licensed|call us at|email us at)/i.test(reply)) || (!catalog && /\$\d/.test(reply))) return json(safe); const safetyLevel = urgentGarageTerms.test([...history.map(item => item.content), message].join("\n")) ? "urgent" : garageIssueTerms.test(message) ? "caution" : "safe"; if (safetyLevel === "urgent" && !/stop|do not|don't|keep .* clear|professional/i.test(reply)) return json(safe); const serviceRequestRecommended = shouldRecommendServiceRequest(message, safetyLevel, reply); reply = withServiceRequestGuidance(reply, serviceRequestRecommended); return json({ reply, safetyLevel, suggestedService: candidate, serviceRequestRecommended }); } catch { return json({ error: "Maya is temporarily unavailable because the AI provider could not be reached.", safetyGuidance: safe.reply, ...safe }, 503); } } catch (e) { return json({ error: e.message }, 400); }
  }
  if (path === "/api/garage/media" && request.method === "POST") { let actor; try { actor = await staffActor(request, env, "admin"); } catch (e) { return json({ error: e.message }, e.status || 401); } if (!env.MEDIA) return json({ error: "Media storage is not configured." }, 503); const type = request.headers.get("content-type")?.split(";")[0].toLowerCase(); const size = Number(request.headers.get("content-length") || 0); if (!SAFE_MEDIA_TYPES.has(type) || size < 1 || size > MAX_MEDIA_BYTES) return json({ error: "Use a JPEG, PNG, or WebP image up to 8 MB." }, 400); const key = `uploads/${crypto.randomUUID()}.${type.split("/")[1]}`; await env.MEDIA.put(key, request.body, { httpMetadata: { contentType: type } }); await audit(env, actor, "media.created", "media", key, ["content-type"]); return json({ key, url: `/api/garage/media/${key}` }, 201); }
  ...
  if (path === "/api/garage/admin/content" || path.startsWith("/api/garage/admin/content/")) {
    let actor; try { actor = await staffActor(request, env, "admin"); } catch (e) { return json({ error: e.message }, e.status || 401); }
    if (!env.DB) return json({ error: "Database is not configured." }, 503);
    if (path === "/api/garage/admin/content" && request.method === "GET") { const result = await env.DB.prepare("SELECT * FROM garage_content ORDER BY sort_order,title").all(); return json(result.results.map(mapContent)); }
    if (path === "/api/garage/admin/content" && request.method === "POST") {
      ...
      } catch (e) { return json({ error: e.message || "Invalid content." }, 400); }
    }
  ...
  if (path === "/api/garage/admin/notifications" || path === "/api/garage/admin/notifications/test") {
    ...
    if (path === "/api/garage/admin/notifications" && request.method === "PUT") {
      ...
      } catch (e) { return json({ error: e.message || "Invalid notification settings." }, 400); }
    }
    if (path.endsWith("/test") && request.method === "POST") {
      try {
        ...
        const response = await fetch(destination, { method: "POST", redirect: "error", headers: { "content-type": "application/json" }, body: JSON.stringify({ event: "notification.test", source: "Cumming Garage Door Service", sentAt: new Date().toISOString() }), signal: AbortSignal.timeout(15000) });
        if (!response.ok) return json({ error: `Destination returned HTTP ${response.status}.` }, 502);
        await env.DB.prepare("UPDATE notification_settings SET tested_at=?,updated_at=? WHERE id=1").bind(new Date().toISOString(), new Date().toISOString()).run();
        await audit(env, actor, "notification_settings.test_delivered", "notification_settings", "1");
        return json({ delivered: true });
      } catch (e) { return json({ error: e.message || "Test delivery failed." }, 502); }
    }
    return json({ error: "Not found" }, 404);
  }
  if (path === "/api/garage/requests" || path.startsWith("/api/garage/requests/") || path === "/api/garage/dashboard" || path === "/api/garage/settings") {
    let actor; try { actor = await staffActor(request, env, path === "/api/garage/settings" ? "admin" : "staff"); } catch (e) { return json({ error: e.message }, e.status || 401); }
    if (!env.DB) return json({ error: "Database is not configured." }, 503);
    if (path === "/api/garage/requests" && request.method === "GET") { const r = await env.DB.prepare("SELECT * FROM service_requests ORDER BY created_at DESC").all(); return json(await Promise.all(r.results.map(async row => ({ ...mapRequest(row), ...await deliveryDetails(env, row.id) })))); }
    ...
    if (path === "/api/garage/settings" && request.method === "PATCH") {
      try {
        ...
        const sensitiveChanged = sensitiveKeys.some(key =>
          ...
          JSON.stringify(update[key]) !== JSON.stringify(currentSettings[key])
        );
        ...
        delete update.verificationStatus; delete update.verified; delete update.verificationAcknowledged;
      ...
      } catch (e) { return json({ error: e.message || "Invalid settings." }, 400); }
    }
    return json({ error: "Not found" }, 404);
  }
  return json({ error: "Not found" }, 404);
...
function withAssetHeaders(response, assetPath) { const headers = new Headers(response.headers); const versioned = /^\/assets\/.+-[A-Za-z0-9_-]{8,}\.[a-z0-9]+$/i.test(assetPath); headers.set("cache-control", assetPath === "/index.html" ? "no-store" : versioned ? "public, max-age=31536000, immutable" : "public, max-age=300, must-revalidate"); headers.set("content-security-policy", "frame-ancestors 'self' https://creativecoders.tech https://*.creativecoders.tech"); headers.set("x-content-type-options", "nosniff"); headers.set("x-robots-tag", "noindex, nofollow, noarchive"); headers.set("referrer-policy", "strict-origin-when-cross-origin"); return new Response(response.body, { status: response.status, statusText: response.statusText, headers }); }
...
export async function sitePublication(env, requestUrl) {
  ...
  async fetch(request, env, context) {
    ...
    try {
      ...
      if ((request.method === "GET" || request.method === "HEAD") && publicPath === "/robots.txt") {
        const settings = await sitePublication(env, request.url);
        return new Response(request.method === "HEAD" ? null : renderRobots(request.url, settings), { headers: { "content-type": "text/plain; charset=utf-8", "cache-control": "no-store", "x-robots-tag": "noindex" } });
      }
      if ((request.method === "GET" || request.method === "HEAD") && publicPath === "/sitemap.xml") {
        const [content, settings] = await Promise.all([getPublicContent(env), sitePublication(env, request.url)]);
        return new Response(request.method === "HEAD" ? null : renderSitemap(request.url, content, settings), { headers: { "content-type": "application/xml; charset=utf-8", "cache-control": "no-store", "x-robots-tag": "noindex" } });
      }
