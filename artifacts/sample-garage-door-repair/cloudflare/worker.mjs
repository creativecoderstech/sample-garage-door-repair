...
async function getCustomerCareContext(env) {
  let row = null, catalog = [];
  try { if (env.DB) { row = await env.DB.prepare("SELECT settings_json, verified FROM business_settings WHERE id=1").first(); const result = await env.DB.prepare("SELECT * FROM services WHERE verified=1 ORDER BY id").all(); catalog = result.results.map(mapService); } } catch { row = null; catalog = []; }
  const settings = publicSettings(row), businessVerified = settings.verificationStatus === "verified", catalogVerified = catalog.length > 0;
  const serviceLines = catalogVerified ? catalog.map(service => `- ${service.name}: ${service.description} Published starting estimate $${service.startingPrice}; typical duration ${service.duration}. Final price and timing require confirmation.`).join("\n") : "- No service catalog is verified. Discuss general garage-door guidance and the request process without claiming the business offers a specific service.";
  const faqLines = customerCareFaqs.map(([question, answer]) => `- ${question}: ${answer}`).join("\n");
  return { settings, catalog, text: ["AUTHORITATIVE WEBSITE AND BUSINESS CONTEXT — use only these facts.", `Business: ${settings.businessName}`, `Business profile verification: ${businessVerified ? "verified" : "unverified"}.`, `Phone: ${settings.phone || "not verified"}`, `Email: ${settings.email || "not verified"}`, `Service area: ${settings.serviceArea}`, `Hours: ${settings.trustProfile?.hours || "not verified"}`, `Warranty: ${settings.trustProfile?.warranty || "not verified"}`, `License and insurance: ${settings.trustProfile?.licenseInsurance || "not verified"}`, "A service request is reviewed by the business and is not a confirmed appointment. Coverage, timing, and final price require confirmation.", `Service catalog verification: ${catalogVerified ? "verified" : "not verified"}.`, "Services:", serviceLines, "Approved customer guidance:", faqLines, "Do not invent reviews, ratings, hours, appointment slots, guarantees, warranties, credentials, refunds, final prices, urgent availability, or coverage. If a detail is absent or unverified, say so and recommend a service request."].join("\n") };
}
