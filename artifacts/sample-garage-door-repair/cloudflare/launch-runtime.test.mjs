...
const configured = {
  ...
  AI: { run() {} }, ASSETS: { fetch() {} },
  CLERK_PUBLISHABLE_KEY: "pk_live_fixture", CLERK_SECRET_KEY: "sk_live_fixture",
...
test("launch runtime fails closed for previews, missing bindings, development auth and test anti-bot keys", () => {
  ...
  for (const changes of [
    ...
    { ASSETS: undefined }, { CLERK_SECRET_KEY: "sk_test_fixture" },
...
test("public checks never return credential values", () => {
  ...
  assert.ok(!output.includes(configured.CLERK_SECRET_KEY));
