...
function GoogleSignIn({ onCredential }: { onCredential: (token: string) => void }) {
  ...
  return <div className="w-full max-w-md rounded-[var(--phi-radius)] bg-white p-[var(--phi-space-4)] text-center shadow-xl">
    <h1 className="text-2xl font-bold text-slate-950">Staff access</h1>
    <p className="mt-[var(--phi-space-2)] text-sm text-slate-600">Continue with your authorized Google account.</p>
    <div ref={target} className="mt-[var(--phi-space-4)] flex justify-center" />
    {message && <p className="mt-[var(--phi-space-3)] text-sm text-red-700">{message}</p>}
  </div>;
}
...
function StaffGate() {
  ...
  if (error) return <AuthShell><div className="w-full max-w-md rounded-[var(--phi-radius)] bg-white p-[var(--phi-space-4)] text-center shadow-xl">
    <ShieldAlert className="mx-auto h-10 w-10 text-orange-700" /><h1 className="mt-[var(--phi-space-3)] text-2xl font-bold text-slate-950">Staff access required</h1>
    <p className="mt-[var(--phi-space-2)] text-sm leading-6 text-slate-600">{error.message}</p>
    <div className="mt-[var(--phi-space-4)] flex justify-center gap-[var(--phi-space-2)]"><Button variant="outline" onClick={() => void refresh()}>Check again</Button><Button onClick={signOut}><LogOut className="mr-2 h-4 w-4" /> Sign out</Button></div>
  </div></AuthShell>;
...
function AuthShell({ children }: { children: React.ReactNode }) {
  return <main className="flex min-h-[100dvh] items-center justify-center bg-slate-950 px-[var(--phi-space-3)] py-[var(--phi-space-5)]"><div className="absolute left-[var(--phi-space-4)] top-[var(--phi-space-4)] flex items-center gap-[var(--phi-space-1)] text-white"><Building2 className="h-6 w-6 text-orange-500" /><span className="font-bold">Cumming Garage Door Service</span></div>{children}</main>;
}
